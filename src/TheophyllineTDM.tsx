...
export default function TheophyllineTDM(){
  ...
  const timingValid=steady&&!!hoursAfter;
  ...
  const note=["=== Theophylline TDM Note ===",`Formulation: ${formulation}; current dose: ${dailyDose||"--"} mg/day.`,`Serum theophylline: ${level||"--"} mcg/mL, drawn ${hoursAfter||"--"} hr after the last dose.`,`Steady state: ${steady?"reported":"not confirmed"}. Toxicity symptoms: ${symptoms?"present":"not reported"}.`,`Assessment: ${assessmentEn}`,`Interaction/context: ${contextEn}`,result.estimate&&!symptoms?`Dose-planning estimate: approximately ${result.estimate} mg/day; verify formulation, timing, interaction trajectory, and clinical response before use.`:""].filter(Boolean).join("\n");
  ...
  return <div>
    ...
    <Card title="目前治療與採血"><div style={S.grid2}>
      ...
      <Field label="距最後一劑多久抽血？" hint="hours"><input value={hoursAfter} onChange={e=>setHoursAfter(e.target.value)} inputMode="decimal" style={S.input}/></Field>
